...
test(set.name, (t) => {
	...
	t.true(
		s.symmetric_difference('de').isequal('abcde'),
		'symmetric_difference abcde',
	);
	t.true(s.isequal('abc'));

	t.true(
		s.symmetric_difference('bcd').isequal('ad'),
		'symmetric_difference ad',
	);
	t.true(s.isequal('abc'));
	...
	t.true(s.isequal('abcde'), 'update ...');
	t.true(
		s.clear().update('abc').update('d', 'e').isequal('abcde'),
		'chain update ...',
	);

	s = set('abc');
	s.intersection_update('de');
	t.true(s.isequal(''), 'intersection update empty');
	t.true(
		s.clear().update('abc').intersection_update('de').isequal(''),
		'chain intersection update empty',
	);

	s = set('abc');
	s.intersection_update('abc', 'de');
	t.true(s.isequal(''), 'intersection update ... empty');
	t.true(
		s.clear().intersection_update('abc', 'de').isequal(''),
		'chain intersection update ... empty',
	);

	s = set('abc');
	s.intersection_update('bcd');
	t.true(s.isequal('bc'), 'intersection update bc');
	t.true(
		s.clear().update('abc').intersection_update('bcd').isequal('bc'),
		'chain intersection update bc',
	);

	s = set('abc');
	s.intersection_update('abcd', 'bcd');
	t.true(s.isequal('bc'), 'intersection update ... bc');
	t.true(
		s.clear().update('abc').intersection_update('abcd', 'bcd').isequal('bc'),
		'chain intersection update ... bc',
	);
	s = set('abc');
	s.difference_update('de');
	t.true(s.isequal('abc'), 'difference_update abc');
	t.true(
		s.clear().update('abc').difference_update('de').isequal('abc'),
		'chain difference_update abc',
	);

	s = set('abc');
	s.difference_update('d', 'e');
	t.true(s.isequal('abc'), 'difference_update ... abc');
	t.true(
		s.clear().update('abc').difference_update('d', 'e').isequal('abc'),
		'chain difference_update ... abc',
	);

	s = set('abc');
	s.difference_update('bcd');
	t.true(s.isequal('a'), 'difference_update a');
	t.true(
		s.clear().update('abc').difference_update('bcd').isequal('a'),
		'chain difference_update a',
	);

	s = set('abc');
	s.difference_update('bc', 'd');
	t.true(s.isequal('a'), 'difference_update ... a');
	t.true(
		s.clear().update('abc').difference_update('bc', 'd').isequal('a'),
		'chain difference_update ... a',
	);

	s = set('abc');
	s.symmetric_difference_update('de');
	t.true(s.isequal('abcde'), 'symmetric_difference_update abcde');
	t.true(
		s.clear().update('abc').symmetric_difference_update('de').isequal('abcde'),
		'chain symmetric_difference_update abcde',
	);

	s = set('abc');
	s.symmetric_difference_update('bcd');
	t.true(s.isequal('ad'), 'symmetric_difference_update ad');
	t.true(
		s.clear().update('abc').symmetric_difference_update('bcd').isequal('ad'),
		'chain symmetric_difference_update ad',
	);
	...
	t.deepEqual(
		...
		s,
		'ref symmetric_difference_update',
	);
});
